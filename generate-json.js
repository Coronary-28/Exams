...
function parseQuestionFile(raw, meta) {
   ...
   for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
     ...
     // ✅ استخراج الصورة من النص
     const imgMatch = questionText.match(/\[IMG:([^\]]+)\]/i);
     if (imgMatch) {
       imageUrl = imgMatch[1];
       questionText = questionText.replace(/\[IMG:[^\]]+\]/i, '').trim();
     }
     ...
     questions.push({
       ...
       text: questionText,
       imageUrl: imageUrl || null,  // ✅ إضافة الحقل الجديد
       options,
